...
function dev_scripts() {
	...
	// wp_enqueue_script( 'devkai-navigation', THEME_URI . '/js/navigation.js', '', '', true );
...
function wp_bootstrap_pagination( $args = array() ) {
    ...
    if ($next && ($count != $page) )
        $echo .= '<li class="navi-next"><a href="' . $next . '" title="' . __( '›', 'devkai') . '">' . $args['next_string'] . '</a></li>';
...
function devkai_metabox_project()
{
...
add_action('add_meta_boxes', 'devkai_metabox_project');



// show custom field bds
function devkai_render_box_project($post)
{
    global $wpdb;
...
function devkai_save_box_project($post_id, $post, $update)
{
...
add_action('save_post', 'devkai_save_box_project', 10, 3);
